...
Image::Image(vk::Extent3D extent, vk::Format format, vk::ImageUsageFlags usageFlags,
             VmaMemoryUsage memoryUsage, vk::SampleCountFlagBits sampleCount, uint32_t mipLevels,
             uint32_t arrayLayers, vk::ImageTiling tiling, vk::ImageCreateFlags flags)
    : mExtent(extent), mFormat(format), mUsageFlags(usageFlags), mSampleCount(sampleCount),
      mMipLevels(mipLevels), mArrayLayers(arrayLayers), mTiling(tiling) {
  vk::ImageCreateInfo imageInfo(flags, findImageTypeFromExtent(mExtent), format, extent, mipLevels,
                                arrayLayers, sampleCount, tiling, usageFlags);
...
void Image::uploadLevel(void const *data, size_t size, uint32_t arrayLayer, uint32_t mipLevel) {
  ...
  vk::BufferImageCopy copyRegion(
      ...
      vk::ImageSubresourceLayers(getImageAspectFlags(mFormat), mipLevel, arrayLayer, 1),
...
void Image::upload(void const *data, size_t size, uint32_t arrayLayer, bool mipmaps) {
  ...
  vk::BufferImageCopy copyRegion(
      ...
      vk::ImageSubresourceLayers(getImageAspectFlags(mFormat), 0, arrayLayer, 1),
      vk::Offset3D(0, 0, 0), mExtent);

  auto pool = mContext->createCommandPool();
  auto cb = pool->allocateCommandBuffer();
  cb->beginOneTime();
  transitionLayout(cb->getInternal(), vk::ImageLayout::eUndefined,
  ...
  cb->copyBufferToImage(stagingBuffer->getVulkanBuffer(), mImage,
                        vk::ImageLayout::eTransferDstOptimal, copyRegion);
  if (mipmaps) {
    generateMipmaps(cb->getInternal(), arrayLayer);
  } else {
...
void Image::copyToBuffer(vk::Buffer buffer, size_t size, vk::Offset3D offset, vk::Extent3D extent,
  ...
  auto cb = pool->allocateCommandBuffer();
  cb->beginOneTime();
  recordCopyToBuffer(cb->getInternal(), buffer, 0, size, offset, extent, arrayLayer);
  cb->end();
  if (cb->submitAndWait() != vk::Result::eSuccess) {
...
void Image::download(void *data, size_t size, vk::Offset3D offset, vk::Extent3D extent,
  ...
  if (mCurrentLayout == vk::ImageLayout::eGeneral) {
    // wait for everything in general layout
    transitionLayout(cb->getInternal(), vk::ImageLayout::eGeneral, vk::ImageLayout::eGeneral,
                     vk::AccessFlagBits::eMemoryWrite, vk::AccessFlagBits::eTransferRead,
                     vk::PipelineStageFlagBits::eAllCommands,
                     vk::PipelineStageFlagBits::eTransfer);
  } else if (mCurrentLayout != vk::ImageLayout::eTransferSrcOptimal) {
    ...
    transitionLayout(cb->getInternal(), sourceLayout, vk::ImageLayout::eTransferSrcOptimal,
...
void Image::transitionLayout(vk::CommandBuffer commandBuffer, vk::ImageLayout oldImageLayout,
  ...
  vk::ImageSubresourceRange imageSubresourceRange(getImageAspectFlags(mFormat), 0, mMipLevels,
                                                  arrayLayer, 1);
...
void Image::transitionLayout(vk::CommandBuffer commandBuffer, vk::ImageLayout oldImageLayout,
  ...
  vk::ImageSubresourceRange imageSubresourceRange(getImageAspectFlags(mFormat), 0, mMipLevels, 0,
                                                  mArrayLayers);
  vk::ImageMemoryBarrier barrier(sourceAccessMask, destAccessMask, oldImageLayout, newImageLayout,
                                 VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, mImage,
                                 imageSubresourceRange);
