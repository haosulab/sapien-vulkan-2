...
bool DenoiserOptix::init(OptixPixelFormat pixelFormat, bool albedo, bool normal, bool hdr) {
...
void DenoiserOptix::denoise(core::Image &color, core::Image *albedo, core::Image *normal) {
  ...
  mCommandBufferIn->beginOneTime();

  color.recordCopyToBuffer(mCommandBufferIn->getInternal(), mInputBuffer->getVulkanBuffer(), 0,
                           mInputBuffer->getSize(), {0, 0, 0}, color.getExtent());

  if (useAlbedo() && albedo) {
    albedo->recordCopyToBuffer(mCommandBufferIn->getInternal(), mAlbedoBuffer->getVulkanBuffer(),
                               0, mAlbedoBuffer->getSize(), {0, 0, 0}, color.getExtent());
  }
  if (useNormal() && normal) {
    normal->recordCopyToBuffer(mCommandBufferIn->getInternal(), mNormalBuffer->getVulkanBuffer(),
                               0, mNormalBuffer->getSize(), {0, 0, 0}, color.getExtent());
  }

  mCommandBufferIn->end();

  core::Context::Get()->getQueue().submit(mCommandBufferIn->getInternal(), {}, {}, {}, mSem.get(),
                                          ++mSemValue, {});
  ...
  mCommandBufferOut->beginOneTime();

  color.recordCopyFromBuffer(mCommandBufferOut->getInternal(), mOutputBuffer->getVulkanBuffer(), 0,
                             mOutputBuffer->getSize(), {0, 0, 0}, color.getExtent());

  mCommandBufferOut->end();

  vk::PipelineStageFlags waitStage = vk::PipelineStageFlagBits::eTransfer;
  core::Context::Get()->getQueue().submit(mCommandBufferOut->getInternal(), mSem.get(), waitStage,
                                          mSemValue, {}, {}, {});
}
