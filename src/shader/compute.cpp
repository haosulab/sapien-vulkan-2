...
std::unique_ptr<core::Image> generateBRDFLUT(uint32_t size) {
  ...
  cb->beginOneTime();
  image->transitionLayout(cb->getInternal(), vk::ImageLayout::eUndefined,
  ...
  cb->bindDescriptorSet(vk::PipelineBindPoint::eCompute, pipelineLayout.get(), 0,
                        descriptorSet.get());
  cb->dispatch(size, size, 1);
  image->transitionLayout(
      cb->getInternal(), vk::ImageLayout::eGeneral, vk::ImageLayout::eShaderReadOnlyOptimal,
...
void prefilterCubemap(core::Image &image) {
  ...
  for (uint32_t level = 1; level < levels; ++level) {
    ...
    cb->pushConstants(pipelineLayout.get(), vk::ShaderStageFlagBits::eCompute, 0, sizeof(float),
                      &roughness);
    cb->bindDescriptorSet(vk::PipelineBindPoint::eCompute, pipelineLayout.get(), 1,
