...
layout(set = 1, binding = 2) uniform sampler2D samplerRandom512x512_0;
layout(set = 1, binding = 3) uniform sampler2D samplerRandom512x512_1;
...
vec3 sampleHemisphere(vec2 coord, int index, int width, int height) {
  float z = texture(samplerRandom512x512_0, coord * vec2(N_SAMPLE * width / 512.f + index,
                                            height / 512.f)).x;
  float u = 6.283185307179586 *
            texture(samplerRandom512x512_1, coord * vec2(N_SAMPLE * width / 512.f + index,
                                            height / 512.f)).x;
  float xy = sqrt(1-z*z);
